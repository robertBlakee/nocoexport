...
def list_bases(base_src_path):
    ...
    try:
        ...
        bases_list = pd.read_sql_query("SELECT title FROM nc_bases_v2 WHERE deleted='0'", base_src)
    except Exception:
...
def export_base(base_src_path, base_title):
    # Try connect to base and check data propriety
    try:
        base_src_url = "file:{}?mode=rw".format(pathname2url(base_src_path))
        base_src = sqlite3.connect(base_src_url, uri=True)
    ...
    base_ID = pd.read_sql_query(f"SELECT id FROM nc_bases_v2 WHERE title='{base_title}' AND deleted='0'", base_src)['id'][0]
    if base_ID == "":
    ...
    try:
        ...
        export_engine(base_src, base_exp, base_ID)
...
def import_base(base_exp_path, base_tar_path):
    # Try connect to base and check data propriety
    try:
    ...
    answer = pd.read_sql_query(f"SELECT id FROM nc_bases_v2 WHERE title='{base_exp_title}' WHERE deleted='0'", base_tar)
    if not answer.empty:
...
def move_base(base_src_path, base_tar_path, base_title):
    ...
    if base_ID == "":
        ...
        return
    # Check base with the same title exists in target file
    answer = pd.read_sql_query(f"SELECT id FROM nc_bases_v2 WHERE title='{base_title}' AND deleted='0'", base_tar)
    if not answer.empty:
        print("Base with given title already exists in target file")
        return
